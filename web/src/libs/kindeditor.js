...
_extend(KUploadButton, {
	...
	submit : function() {
        ...
        var fd = new FormData(self.form[0]);
        var xhr = new XMLHttpRequest();
        //xhr.open("POST", CONFIG.server + "b/upload?token=" + sessionStorage.getItem('token'));
        xhr.open("POST", self.form[0].action);
        xhr.responseType = "json";
        xhr.onreadystatechange = function () {
            if (xhr.status == 200 && xhr.response.resultMsg) {
                self.options.afterUpload.call(self, xhr.response);
        ...
        // ================通过FormData的方式上传到OSS================
        var fd = new FormData(self.form[0]);
        var fileBlob = fd.get('file');
        fd.delete('file');
        var ossSignServerAddress = 'http://prod.xiaomaiketang.com:8089/xm/oss/web/token';

        var xhr = new XMLHttpRequest();
        xhr.open('GET', ossSignServerAddress, false);
        xhr.send(null);
        var fileName = random_string(16);
        var signInfo = JSON.parse(xhr.responseText);
        ...
        var dir = 'kindEditorFile/';
        fd.append('key', signInfo.dir + dir + fileName);
        fd.append('file', fileBlob);

        xhr.open('POST', signInfo.host, false);
        xhr.send(fd);
        var result = JSON.parse(xhr.responseText);
        console.log(1);
        self.options.afterUpload.call(self, result.url);
        // ================通过FormData的方式上传到OSS================
